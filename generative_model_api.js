...
function getChat() {
    return Chat.from([
        ...
        { role: "user", content: `В следующем сообщении я пришлю тебе шесть выпавших карт, их значения и символы в определенном порядке в формате JSON. Первые три карты отвечают за утро, следующие две отвечают за день, последняя отвечает за вечер. По этой информации тебе необходимо построить прогноз длиной в 2-3 предложения на время суток. Используй смайлики в предсказаниях. Свой ответ тебе следует начинать с фразы "НАЧАЛО ПРЕДСКАЗАНИЯ". В конце необходимо сделать вывод по предсказанию. Задача ясна?` }
    ])
}
